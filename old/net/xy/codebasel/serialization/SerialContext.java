...
public class SerialContext {
	...
	public SerialContext(final Class[] classes) {
		...
		classes: for (final Iterator iterator = list.iterator(); iterator
				.hasNext();) {
			final Class cl = (Class) iterator.next();
			if (!cl.isInterface() && !cl.isInstance(Serializable.class)) {
				Class pcl = cl.getSuperclass();
				while (pcl != null) {
					if (pcl.isInstance(Serializable.class)
							|| cl == Serializable.class) {
						continue classes;
					}
		...
		Collections.sort(list, new Comparator() {
			public int compare(final Object c1, final Object c2) {
				return ((Class) c1).getSimpleName().compareTo(
	...
	private void write(final DataOutputStream out, final Object target)
		...
		case -8:
			// List
			out.writeInt(((List) target).size()); // write len
			for (final Iterator iterator = ((List) target).iterator(); iterator
					.hasNext();) {
		...
		case -9:
			// Map
			out.writeInt(((Map) target).size()); // write len
			for (final Iterator iterator = ((Map) target).entrySet().iterator(); iterator
					.hasNext();) {
		...
		default:
			...
			for (final Iterator iterator = fields.iterator(); iterator
					.hasNext();) {
	 ...
	 */
	private Object read(final DataInputStream in)
		...
		case -10:
			...
			if (alength > 0) {
				final Class comp = SerialContext.class.getClassLoader()
	...
	private int getEid(final Class clazz) {
		...
		throw new IllegalStateException(Debug.values("Class not in context",
				new Object[] { clazz }));
	}
}