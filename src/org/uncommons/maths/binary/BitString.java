...
/**
 * <p>
 * Implementation of a fixed-length bit-string. Internally, bits are packed into
 * an array of ints. This implementation makes more efficient use of space than
 * the alternative approach of using an array of booleans.
 * </p>
 * <p>
 * This class is preferable to {@link java.util.BitSet} if a fixed number of
 * bits is required.
 * </p>
 * 
 * @author Daniel Dyer
...
public final class BitString implements Cloneable, Serializable {
	...
	/**
	 * Store the bits packed in an array of 32-bit ints. This field cannot be
	 * declared final because it must be cloneable.
	 */
	private int[] data;

	/**
	 * Creates a bit string of the specified length with all bits initially set
	 * to zero (off).
	 * 
	 * @param length
	 *            The number of bits.
	...
	/**
	 * Creates a bit string of the specified length with each bit set randomly
	 * (the distribution of bits is uniform so long as the output from the
	 * provided RNG is also uniform). Using this constructor is more efficient
	 * than creating a bit string and then randomly setting each bit
	 * individually.
	 * 
	 * @param length
	 *            The number of bits.
	 ...
	 * 
	 * @param index
	 *            The index of the bit to look-up (0 is the least-significant
	 *            bit).
	...
	public int countSetBits() {
		...
		for (int x : data)
			while (x != 0) {
				...
				++count; // Count how many times we have to unset a bit before x
							// equals zero.
			}
		return count;
	}
	...
	/**
	 * An efficient method for exchanging data between two bit strings. Both bit
	 * strings must be long enough that they contain the full length of the
	 * specified substring.
	 * 
	 * @param other
	 *            The bitstring with which this bitstring should swap bits.
	 * @param start
	 *            The start position for the substrings to be exchanged. All bit
	 *            indices are big-endian, which means position 0 is the
	 *            rightmost bit.
	 * @param length
	...
	/**
	 * @param other
	 *            The BitString to exchange bits with.
	 * @param word
	 *            The word index of the word that will be swapped between the
	 *            two bit strings.
	 * @param swapMask
	 *            A mask that specifies which bits in the word will be swapped.
	 */
	...
	/**
	 * Creates a textual representation of this bit string in big-endian order
	...
	/**
	 * @return True if the argument is a BitString instance and both bit strings
	 *         are the same length with identical bits set/unset.
	 */
	@Override
	...
	public int hashCode() {
		int result = length;
		result = 31 * result + Primitive.hashCode(data);
		return result;
	}
}
